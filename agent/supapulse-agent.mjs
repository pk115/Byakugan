...
async function pollScanJob(){
  if(scanRunning)return;scanRunning=true;let job,heartbeat;
  try{
    const response=await request("/api/agent/scan-jobs/next");if(!response.ok)throw new Error(`Job poll returned HTTP ${response.status}`);job=await response.json();if(!job||job.job===null)return;
    await request(`/api/agent/scan-jobs/${job.id}/progress`,{method:"POST",body:JSON.stringify({progress:15})});
    heartbeat=setInterval(()=>request(`/api/agent/scan-jobs/${job.id}/progress`,{method:"POST",body:JSON.stringify({progress:50})}).catch(()=>undefined),30000);
    const started=Date.now(),subcommand={FILESYSTEM:"fs",ROOTFS:"rootfs",IMAGE:"image"}[job.targetType];if(!subcommand)throw Object.assign(new Error("Unsupported scan target type"),{code:"UNSUPPORTED_TARGET"});
    const version=(await exec("trivy",["--version"],{timeout:15000,maxBuffer:100000})).stdout.split("\n")[0].trim();
    const args=[subcommand,"--quiet","--scanners",job.scanners.join(","),"--severity",job.severity.join(","),"--format","json",job.target];
    const output=(await exec("trivy",args,{timeout:Math.max(60000,Number(job.timeoutSeconds??900)*1000),maxBuffer:50_000_000})).stdout;
    clearInterval(heartbeat);heartbeat=undefined;
    await request(`/api/agent/scan-jobs/${job.id}/progress`,{method:"POST",body:JSON.stringify({progress:90})});
    const normalized=normalizeTrivy(JSON.parse(output)),result=await request(`/api/agent/scan-jobs/${job.id}/result`,{method:"POST",body:JSON.stringify({scanner:"Trivy",scannerVersion:version,observedAt:new Date().toISOString(),findings:normalized.findings,summary:{durationMs:Date.now()-started,truncated:normalized.truncated}})});if(!result.ok)throw new Error(`Result upload returned HTTP ${result.status}: ${await result.text()}`);
    console.log(`${new Date().toISOString()} scan job ${job.id} completed with ${normalized.findings.length} finding(s)`)
  }catch(error){console.error(new Date().toISOString(),error.message);if(job?.id)await request(`/api/agent/scan-jobs/${job.id}/failure`,{method:"POST",body:JSON.stringify({code:error.code??(error.code==="ENOENT"?"TRIVY_NOT_INSTALLED":"SCAN_FAILED"),message:String(error.message??"Scan failed").slice(0,1000)})}).catch(()=>undefined)}finally{if(heartbeat)clearInterval(heartbeat);scanRunning=false}
}
