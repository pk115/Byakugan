...
const scanResultSchema=z.object({scanner:z.literal("Trivy"),scannerVersion:z.string().max(100),observedAt:z.string().datetime(),findings:z.preprocess(value=>Array.isArray(value)?value.filter(item=>{if(!item||typeof item!=="object")return false;const finding=item as Record<string,unknown>;return [finding.id,finding.packageName,finding.title].some(field=>typeof field==="string"&&field.trim().length>0)}):value,z.array(scanFindingSchema).max(5000)),summary:z.object({durationMs:z.number().int().nonnegative(),truncated:z.boolean().default(false)})});
...
app.get("/api/vulnerability-scan-jobs",{preHandler:requireAuth},async()=>db.prepare(`SELECT j.id,j.agent_id AS agentId,a.name AS agentName,a.hostname,j.profile_id AS profileId,j.schedule_id AS scheduleId,j.target_type AS targetType,j.target,j.scanners,j.severity,j.timeout_seconds AS timeoutSeconds,j.status,j.progress,j.requested_at AS requestedAt,j.started_at AS startedAt,j.completed_at AS completedAt,j.error_code AS errorCode,j.error_message AS errorMessage,j.result_summary_json AS resultSummary FROM vulnerability_scan_jobs j JOIN agents a ON a.id=j.agent_id WHERE j.archived_at IS NULL ORDER BY j.requested_at DESC LIMIT 200`).all().map(row=>({...row as Record<string,unknown>,scanners:String((row as {scanners:string}).scanners).split(","),resultSummary:(row as {resultSummary:string|null}).resultSummary?JSON.parse((row as {resultSummary:string}).resultSummary):null})));
app.get("/api/vulnerability-scan-jobs/:id",{preHandler:requireAuth},async(request,reply)=>{const id=Number((request.params as {id:string}).id),job=db.prepare(`SELECT j.id,j.agent_id AS agentId,a.name AS agentName,a.hostname,j.target_type AS targetType,j.target,j.scanners,j.severity,j.status,j.progress,j.requested_at AS requestedAt,j.started_at AS startedAt,j.completed_at AS completedAt,j.error_code AS errorCode,j.error_message AS errorMessage,j.result_summary_json AS resultSummary FROM vulnerability_scan_jobs j JOIN agents a ON a.id=j.agent_id WHERE j.id=?`).get(id) as Record<string,unknown>|undefined;if(!job)return reply.code(404).send({error:"Scan job not found"});const items=db.prepare(`SELECT vulnerability_id AS id,package_name AS packageName,installed_version AS installedVersion,fixed_version AS fixedVersion,severity,title,finding_type AS type,resource_path AS resourcePath,primary_url AS primaryUrl,status FROM vulnerability_findings WHERE agent_id=? AND last_scan_job_id=? ORDER BY CASE severity WHEN 'CRITICAL' THEN 1 WHEN 'HIGH' THEN 2 WHEN 'MEDIUM' THEN 3 WHEN 'LOW' THEN 4 ELSE 5 END,vulnerability_id LIMIT 5000`).all(job.agentId,id);return{...job,scanners:String(job.scanners).split(","),resultSummary:job.resultSummary?JSON.parse(String(job.resultSummary)):null,findings:items}});
app.post("/api/vulnerability-scan-jobs",{preHandler:requireAuth},async(request,reply)=>{
  const body=parse(scanJobSchema,request.body,reply);if(!body)return;
  const agent=db.prepare(`SELECT a.id,s.os_name AS osName FROM agents a LEFT JOIN asset_snapshots s ON s.id=(SELECT id FROM asset_snapshots WHERE agent_id=a.id ORDER BY observed_at DESC LIMIT 1) WHERE a.id=? AND a.enabled=1`).get(body.agentId) as {id:number;osName:string|null}|undefined;
  if(!agent)return reply.code(404).send({error:"Active agent not found"});
  const profile=body.profileId?db.prepare("SELECT id,target_type AS targetType,scanners,severity,timeout_seconds AS timeoutSeconds FROM vulnerability_scan_profiles WHERE id=? AND enabled=1").get(body.profileId) as {id:number;targetType:string;scanners:string;severity:string;timeoutSeconds:number}|undefined:undefined;
  if(body.profileId&&!profile)return reply.code(404).send({error:"Scan profile not found"});
  const targetType=profile?.targetType??body.targetType,scanners=profile?.scanners??body.scanners.join(","),severity=profile?.severity??body.severity.join(","),timeout=profile?.timeoutSeconds??900;
  if(targetType==="ROOTFS"&&/windows/i.test(agent.osName??""))return reply.code(400).send({error:"ROOTFS scanning is supported on Linux agents only. Use Quick vulnerabilities (FILESYSTEM) or Container image security on Windows."});
  const duplicate=db.prepare("SELECT id FROM vulnerability_scan_jobs WHERE agent_id=? AND target_type=? AND target=? AND status IN ('QUEUED','RUNNING')").get(body.agentId,targetType,body.target) as {id:number}|undefined;
  if(duplicate)return reply.code(409).send({error:"An active scan already exists for this agent and target",jobId:duplicate.id});
  const result=db.prepare("INSERT INTO vulnerability_scan_jobs(agent_id,profile_id,target_type,target,scanners,severity,timeout_seconds,requested_by) VALUES(?,?,?,?,?,?,?,?)").run(body.agentId,profile?.id??null,targetType,body.target,scanners,severity,timeout,actor(request)),id=Number(result.lastInsertRowid);
  appendAuditEvent(actor(request),"VULNERABILITY_SCAN_QUEUED","scan_job",id,{agentId:body.agentId,profileId:profile?.id,targetType,target:body.target,scanners:scanners.split(",")});return reply.code(201).send({id,status:"QUEUED"})
});
...
app.delete("/api/vulnerability-scan-jobs/:id",{preHandler:requireAuth},async(request,reply)=>{const id=Number((request.params as {id:string}).id),job=db.prepare("SELECT id,status FROM vulnerability_scan_jobs WHERE id=? AND archived_at IS NULL").get(id) as {id:number;status:string}|undefined;if(!job)return reply.code(404).send({error:"Visible scan job not found"});if(["QUEUED","RUNNING"].includes(job.status))return reply.code(409).send({error:"Cancel the active scan before removing it from the list"});const now=new Date().toISOString();db.prepare("UPDATE vulnerability_scan_jobs SET archived_at=? WHERE id=?").run(now,id);appendAuditEvent(actor(request),"VULNERABILITY_SCAN_ARCHIVED","scan_job",id,{status:job.status});return reply.code(204).send()});
...
app.get("/api/agents",{preHandler:requireAuth},async()=>db.prepare(`SELECT a.id,a.name,a.hostname,a.agent_version AS agentVersion,a.last_seen_at AS lastSeenAt,a.enabled,a.created_at AS createdAt,
  ...
  LEFT JOIN host_metrics m ON m.id=(SELECT id FROM host_metrics WHERE agent_id=a.id ORDER BY observed_at DESC LIMIT 1) WHERE a.enabled=1 ORDER BY a.name`).all().map(row=>({...(row as Record<string,unknown>),enabled:Boolean((row as {enabled:number}).enabled),rebootRequired:Boolean((row as {rebootRequired:number}).rebootRequired)})));
...
app.delete("/api/agents/:id",{preHandler:requireAuth},async(request,reply)=>{const id=Number((request.params as {id:string}).id);const agent=db.prepare("SELECT id,name,last_seen_at AS lastSeenAt FROM agents WHERE id=? AND enabled=1").get(id) as {id:number;name:string;lastSeenAt:string|null}|undefined;if(!agent)return reply.code(404).send({error:"Active agent not found"});const now=new Date().toISOString();db.transaction(()=>{db.prepare("UPDATE vulnerability_scan_schedules SET enabled=0,updated_at=? WHERE agent_id=? AND enabled=1").run(now,id);db.prepare("UPDATE vulnerability_scan_jobs SET status='CANCELLED',completed_at=? WHERE agent_id=? AND status='QUEUED'").run(now,id);db.prepare("UPDATE agents SET enabled=0,revoked_at=? WHERE id=?").run(now,id)})();appendAuditEvent(actor(request),agent.lastSeenAt?"AGENT_REVOKED_AND_ARCHIVED":"AGENT_ENROLLMENT_REMOVED","agent",id,{name:agent.name,hadEvidence:Boolean(agent.lastSeenAt)});return reply.code(204).send();});
